#!/usr/bin/env bash
# Create an OAuth2 IdP user WITHOUT certa admin role (for UI test OAuth2_login_without_certa_admin).
# Usage: OAUTH2_IDP_ADMIN_USER=admin OAUTH2_IDP_ADMIN_PASSWORD=admin ./create-oauth2-user-noadmin.sh
...
USERNAME="${OAUTH2_NOADMIN_USER:-certa_noadmin}"
PASSWORD="${OAUTH2_NOADMIN_PASSWORD:-noadmin123}"
...
if [ "$HTTP" = "201" ]; then
  echo "User ${USERNAME} created. Use OAUTH2_TEST_USER_NO_ADMIN=${USERNAME} OAUTH2_TEST_PASSWORD_NO_ADMIN=${PASSWORD} for the no-admin test."
  exit 0
fi
